...
function _concat(a, b) {
	...
	if (aHeight !== 1 || bHeight !== 1) {
		if (aHeight === bHeight) {
			a = (0, _util.nodeCopy)(a);
			b = (0, _util.nodeCopy)(b);
			let tuple = _concat((0, _util.last)(a), (0, _util.first)(b));
			let a0 = tuple[0];
			let b0 = tuple[1];
			insertRight(a, a0);
			insertLeft(b, b0);
			...
			a = (0, _util.nodeCopy)(a);
			let tuple = _concat((0, _util.last)(a), b);
			let a0 = tuple[0];
			let b0 = tuple[1];
			insertRight(a, a0);
			...
			b = (0, _util.nodeCopy)(b);
			var tuple = _concat(a, (0, _util.first)(b));
...
function shuffle(a, b, toRemove) {
	...
	while ((fromA = read < aLen, either = fromA ? a[read] : b[read - aLen],either.length % _const.M === 0)) {
		...
		if(fromNewA) newA[read] = either;
		else newB[read - newALen] = either;
		let size = fromNewA ? a.sizes[read] : b.sizes[read - newALen];
		if(!size) {
			size = newA.sizes[read - 1] + _util.length(either);
		}
		if(fromNewA) newA.sizes[read] = size;
		else newB.sizes[read - newALen] = size;
		read++;
	}
	...
	while (read - write - (slot.length > 0 ? 1 : 0) < toRemove && read - a.length < b.length) {
		...
		if (slot.height > 0) {
			...
			for (var i = len; i < len + to - from; i++) {
				...
				sizes[i] = slot[i] === undefined ? 0 : (0, _util.length)(slot[i]);
