...
function _concat(a, b) {
	...
	if (aHeight !== 1 || bHeight !== 1) {
		if (aHeight === bHeight) {
			...
			a = parentise(tuple[right], tuple[right].height + 1);
		}
	}
...
function insertRight(parent, node) {
	...
	parent[index] = node;
	parent.sizes[index] = length(node) + (index > 0 ? parent.sizes[index - 1] : 0);
}
...
function insertLeft(parent, node) {
	...
	if (node.length > 0) {
		...
		var len = length(parent[0]);
		for (let i = 1, l = sizes.length; l > i; i++) {
			sizes[i] = len = (len += length(parent[i]));
		}
...
function calcToRemove(a, b) {
	...
	return (a.length + b.length) - (Math.floor((subLengths - 1) / M) + 1);
}
